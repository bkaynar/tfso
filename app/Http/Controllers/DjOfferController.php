...
class DjOfferController extends Controller
{
    ...
    /**
     * Update offer budget (Place Manager only, pending & not expired)
     */
    public function updateBudget(Request $request, DjOffer $offer)
    {
        $user = Auth::user();

        if (!$user->hasRole('placeManager') || $offer->place_manager_id !== $user->id) {
            abort(403, 'Only the place manager can update this offer.');
        }

        if (!$offer->isPending()) {
            return response()->json([
                'success' => false,
                'message' => 'Only pending offers can have their budget updated.'
            ], 422);
        }
        ...
        if ($offer->isExpired()) {
            return response()->json([
                'success' => false,
                'message' => 'Cannot update budget of an expired offer.'
            ], 422);
        }
        ...
        $offer->update(['budget' => $validated['budget']]);

        return response()->json([
            'success' => true,
            'budget' => $offer->budget,
            'formatted_budget' => number_format($offer->budget, 2, ',', '.')
        ]);
    }
}
