  ...
  <div class="fixed top-4 right-4 z-50">
    <div class="relative">
      <button @click="toggleDropdown"
        class="flex items-center bg-black bg-opacity-60 text-white px-3 py-2 rounded-lg hover:bg-opacity-80 transition-all"
      ...
      <div v-show="isOpen"
        ...
        <button v-for="lang in languages" :key="lang.code" @click="selectLanguage(lang.code)"
          class="w-full flex items-center px-4 py-3 text-white hover:bg-white hover:bg-opacity-10 transition-colors"
          :class="[
            isRTL ? 'space-x-reverse space-x-3' : 'space-x-3',
            { 'bg-white bg-opacity-20': currentLanguage === lang.code }
          ]">
...
import turkiyeFlag from '../assets/turkiye.png'
...
export default {
  ...
  setup() {
    ...
    const getCurrentFlag = () => {
      ...
      return currentLang?.flag || '/src/assets/flag-en.png'
    }
