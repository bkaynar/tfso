                ...
                <div class="border-2 border-dashed border-purple-400 dark:border-purple-600 rounded-lg p-6">
                  <div v-if="existingImages.length > 0 || imagePreviews.length > 0" class="grid grid-cols-2 gap-2 mb-4">
                    <div v-for="(image, index) in existingImages" :key="`existing-${index}`" class="relative">
                      <img :src="image" alt="Existing" class="w-full h-24 object-cover rounded" />
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
              ...
              <div class="flex items-center">
                ...
                <span v-if="props.place.is_premium" class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                  Evet
                </span>
            ...
            <div>
              ...
              <div v-if="props.place.images && props.place.images.length > 0" class="grid grid-cols-2 gap-2">
                <img v-for="(image, index) in props.place.images" :key="index" :src="image" alt="Place image" class="w-full h-24 object-cover rounded" />
              </div>
              <div v-else class="border-2 border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center bg-gray-50 dark:bg-gray-700">
...
const existingImages = ref<string[]>(props.place.images || [])
