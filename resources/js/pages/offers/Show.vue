                ...
                <div class="lg:col-span-2">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center space-x-4">
                                <div
                                    ...
                                    {{ isPlaceManager ? offer.dj.name.charAt(0).toUpperCase() :
                                        offer.place.name.charAt(0).toUpperCase()
                                    }}
                                </div>
                                <div>
                                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                                        {{ isPlaceManager ? `Offer to ${offer.dj.name}` : `Offer from
                                        ${offer.place.name}` }}
                                    </h1>
                                    <p v-if="!isPlaceManager" class="text-gray-600 dark:text-gray-400">
                            ...
                            <div>
                                ...
                                <div v-if="isPlaceManager && offer.status === 'pending' && !isOfferExpired"
                                    ...
                                    <form v-else @submit.prevent="saveBudget" class="flex items-center space-x-2">
                                        <div class="relative">
                                            <span
                                                class="absolute left-2 top-1/2 -translate-y-1/2 text-sm text-gray-500">₺</span>
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div>
                                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">{{ offer.place.name }}
                                </h3>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg h-[600px] flex flex-col">
                        ...
                        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
                            ...
                            <div v-for="message in messages" :key="message.id"
                                ...
                                <div
                                    :class="['flex max-w-[70%]', message.user_id === currentUserId ? 'flex-row-reverse' : 'flex-row']">

                                    <!-- Avatar -->
                                    <div
                                        :class="['flex-shrink-0', message.user_id === currentUserId ? 'ml-2' : 'mr-2']">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div :class="['px-4 py-3 rounded-2xl shadow-sm relative',
                                            ...
                                            <!-- Message text -->
                                            <p class="text-sm whitespace-pre-wrap leading-relaxed">{{ message.message }}
                                            </p>

                                            <!-- Message status for sent messages -->
                                            <div v-if="message.user_id === currentUserId"
                                                class="flex items-center justify-end mt-1">
                                                <svg v-if="message.sending"
                                                    class="w-3 h-3 text-purple-300 animate-pulse" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <circle cx="10" cy="10" r="2" />
                                                </svg>
                                                <svg v-else-if="message.is_read" class="w-4 h-4 text-purple-200"
                                                ...
                                                <svg v-else class="w-4 h-4 text-purple-300" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                                        clip-rule="evenodd" />
                        ...
                        <div class="p-4 border-t border-gray-200 dark:border-gray-700">
                            <form @submit.prevent="sendMessage" class="flex space-x-2">
                                <textarea v-model="newMessage" @keydown.enter.exact.prevent="sendMessage"
                                    ...
                                    placeholder="Type your message... (Shift+Enter for new line)" maxlength="2000"
                                    rows="1" :disabled="isSendingMessage || props.canMessage === false"
                                ...
                                <button type="submit"
                                    :disabled="props.canMessage === false || !newMessage.trim() || isSendingMessage"
                                    ...
                                    <svg v-if="isSendingMessage" class="animate-spin h-4 w-4"
                            ...
                            <p v-else class="text-xs text-red-500 dark:text-red-400 mt-1">Messaging disabled (offer
                                cancelled)</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 bg-white dark:bg-gray-800 shadow rounded-lg p-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div v-if="offer.expires_at"
                                class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                                <div class="w-2 h-2 bg-orange-500 rounded-full mr-2"></div>
                                <span>{{ isOfferExpired ? 'Expired' : 'Expires' }} {{ formatDate(offer.expires_at)
                                    }}</span>
                            </div>
                        </div>
                    </div>
...
const getStatusBadgeClass = (status: string) => {
    const classes: Record<string, string> = {
        ...
        cancelled: 'bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-100'
    }
